...
def run_client (filename): #client socket functions 
    filesize = os.path.getsize(CLIENT_PATH+"/"+filename) #get size of file being transfered
    ...
    s.send(f"{filePath}{space}{filesize}".encode()) #sends file and file size
    # bar = tqdm.tqdm(range(filesize), f"Sending {filePath}", unit="B", unit_scale=True, unit_divisor=256, colour ='#00ff00')#establishes download bar
    with open(filePath, "rb") as f: # opens a filename in binary format
    ...
    print(f"socket closed {time.time()}")
